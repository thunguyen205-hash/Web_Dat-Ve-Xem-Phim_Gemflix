...
Website đặt vé xem phim và quản trị rạp chiếu cho GemFlix. Dự án được xây dựng bằng Vanilla HTML/CSS/JS cho frontend, gọi API trực tiếp đến backend PHP (OOP) đã được deploy sẵn. Có giao diện khách hàng, luồng đặt vé kèm chọn ghế trực quan, quản trị nội dung phim, lịch chiếu và quản lý doanh thu.

---

## Demo trực tuyến

Dự án giao diện (Frontend) đang sử dụng trực tiếp API được deploy tại:
**API Endpoint**: `http://gemflix.rf.gd/backend/api`
...
## Công nghệ sử dụng
- **Frontend**: HTML5, CSS3 (Vanilla, Responsive Layout).
- **JavaScript**: ES6+, thao tác DOM, xử lý sự kiện, fetch dữ liệu từ API (`Fetch API`).
- **Icons & Fonts**: FontAwesome.
- **Backend (Remote)**: PHP (RESTful API, OOP), MySQL.
...
> **Lưu ý**: Dự án giao diện này đang gọi trực tiếp tới Backend API đã được deploy sẵn trên host `http://gemflix.rf.gd/backend/api`. Bạn không cần cài đặt cơ sở dữ liệu nội bộ.
...
Backend API đã có sẵn cơ sở dữ liệu trên máy chủ. Bạn có thể tự **Đăng ký** một tài khoản người dùng mới và trải nghiệm luồng đặt vé.
...
Khuyến nghị triển khai (hosting) Frontend trên các nền tảng static hosting miễn phí và phổ biến:
